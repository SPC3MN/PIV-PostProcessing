...
def _frame_components(frame):
    """Extract (X, Y, U, V, W) from an lvpyio Frame. lvpyio's exact attribute names
    can vary slightly by version — if this raises against your installed version,
    inspect `frame.components` / `frame.scales` (or `dir(frame)`) and adjust the
    lookups here."""
    comps = frame.components
    U = _masked_to_nan(comps['U0'])
    V = _masked_to_nan(comps['V0'])
    W = _masked_to_nan(comps['W0'])
...
def load_single_vc7(buffer, Ny, Nx):
    X, Y, U, V, W = _frame_components(buffer[0])
    return U[:Ny, :Nx], V[:Ny, :Nx], W[:Ny, :Nx]


def load_dataset_vc7(path, cutoff):
    """Load snapshots directly from DaVis .set/.vc7 files via lvpyio, skipping the
    CSV export/parse round trip entirely."""
    buffers = _vc7_buffers(path, cutoff)
    print(f'Loading {len(buffers)} Files (lvpyio)... ')

    X0, Y0, U0, _, _ = _frame_components(buffers[0][0])
    Ny, Nx = U0.shape
    ...
    with ThreadPoolExecutor() as ex:
        results = list(ex.map(lambda b: load_single_vc7(b, Ny, Nx), buffers))
...
def load_dataset(csv_dir, cutoff, input_format='auto'):
    ...
    if input_format == 'npz':
        ...
        return load_dataset_vc7(csv_dir, cutoff)

...
for case_name, input_dir in case_dirs.items():
    ...
    output_Lum_dir = os.path.join(processed_root, case_name, "Lumley_Statistics")
    ...
    start = time.perf_counter()
    U_mean, V_mean, W_mean, U_fluct, V_fluct, W_fluct, U_rms, V_rms, W_rms, TKE, uv, uw, vw = \
        reynolds_decomp_stereo(U_all, V_all, W_all)
    ...
    if Bootstrap:
        ...
        print(f'TKE: {np.nanmean((U_rms**2+V_rms**2+W_rms**2)/2)} '
              f'({np.nanmean((U_rms_lo**2+V_rms_lo**2+W_rms_lo**2)/2)}, '
              f'{np.nanmean((U_rms_hi**2+V_rms_hi**2+W_rms_hi**2)/2)})')

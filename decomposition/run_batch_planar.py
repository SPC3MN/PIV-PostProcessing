"""
Non-interactive batch runner for Planar_Decomposition, reusing the repo's own
common/* functions unmodified (skips the interactive input() prompts in
Planar_Decomposition.py so it can run over multiple recordings unattended).

For each recording folder under RAW_ROOT (a folder of per-case snap_*.npz
subfolders), runs the full decomposition (bootstrap, structure function,
autocorrelation, spectra -- all repo defaults) with no cropping, and writes
results to OUT_ROOT/<recording_name>_planar (processed)/<case_name>/Ensemble_Averages/.
"""
import os
import sys
import time
...
if __name__ == "__main__":
    ...
    ap.add_argument("--only-recording", type=str, default=None, help="process only this recording folder name")
    ap.add_argument("--only-case", type=str, default=None, help="process only this case name (within the selected recording(s))")
    args = ap.parse_args()
    ...
    if args.only_recording:
        recordings = [r for r in recordings if r == args.only_recording]

    print(f"Found {len(recordings)} recording(s): {', '.join(recordings)}")

    for recording_name in recordings:
        raw_recording_dir = os.path.join(RAW_ROOT, recording_name)
        out_recording_dir = os.path.join(OUT_ROOT, f"{recording_name}_planar (processed)")

        case_dirs = discover_case_dirs_or_root(raw_recording_dir, required_glob="*.npz")
        if args.only_case:
            case_dirs = {k: v for k, v in case_dirs.items() if k == args.only_case}

        print(f"\n##### {recording_name}: {len(case_dirs)} case(s): {', '.join(case_dirs)} #####")

        for case_name, input_dir in case_dirs.items():
            print(f"\n===== {recording_name} / {case_name} =====")
            output_avg_dir = os.path.join(out_recording_dir, case_name, "Ensemble_Averages")

...
the same window Stereo_Analysis.py reuses for both L_11 and L_33) and
...
for case_name, case_dir in case_dirs.items():
    ...
    X = ds["X"]
    dr = (X[0, 1] - X[0, 0]) / 10
    D_11 = ds["D11"][1:-10] * conv
    D_33 = ds["D33"][1:-10] * conv

    da = np.load(auto_file)
    rho_11 = da["rho11"][1:-1]
    rho_11_low = da["rho11_low"][1:-1]
    rho_11_hi = da["rho11_hi"][1:-1]
    rho_33 = da["rho33"][1:-1]
    rho_33_low = da["rho33_low"][1:-1]
    rho_33_hi = da["rho33_hi"][1:-1]

    r1_D = np.arange(0, len(D_11)) * dr
    r3_D = np.arange(0, len(D_33)) * dr
    max_beg11, max_end11 = inertial_range_mask(D_11, r1_D, SLOPE_LO, SLOPE_HI, smooth_sigma=SMOOTH_SIGMA)
    ...
    ######  L_33  (note: reuses the D_11-derived mask window, matching Stereo_Analysis.py)

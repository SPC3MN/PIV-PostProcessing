"""Standalone: mean flow / TKE contour + summary stats for planar cases.

Loads only Averages.npz per case -- no structure function, autocorrelation,
or integral-length computation needed.
"""
import os
import sys
import numpy as np
...
for case_name, case_dir in case_dirs.items():
    ...
    X, Y, U_mean, V_mean, U_rms, V_rms, TKE = (
        d["X"], d["Y"], d["U_mean"], d["V_mean"], d["U_rms"], d["V_rms"], d["TKE"])
    ...
    print(f'Mean U: {U_mean_avg} cm/s   Mean V: {V_mean_avg} cm/s')
    print(f'RMS U: {U_rms_avg} cm/s   RMS V: {V_rms_avg} cm/s')
    print(f'Isotropy ratio (U_rms/V_rms): {Isotropy_ratio}')
    print(f'M1: {M1}')
